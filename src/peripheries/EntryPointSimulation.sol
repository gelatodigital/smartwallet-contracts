...
abstract contract EntryPointSimulation is EntryPoint {
    // TODO: improve accuracy
    function simulateHandleOps(PackedUserOperation[] calldata userOps, address beneficiary)
        external
    {
        (beneficiary);
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < count; i++) {
                _executeUserOp(0, userOps[i], opInfos[i]);
            }
        }
    }
    ...
    function executeUserOp(PackedUserOperation calldata userOp) external {
        ...
        if (userOp.callData.length > 0) {
            ...
            if (!success) {
                // TODO: handle same way EntryPoint would
                assembly {
